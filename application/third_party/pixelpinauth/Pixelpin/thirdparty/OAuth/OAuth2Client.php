...
class OAuth2Client
{
	...
	*/
	public function api( $url, $method = "GET", $parameters = array() ) 
	{
		...
		$stringHttpHeader = implode("|", $httpHeader);

		switch( $method ){
			case 'GET'  : $response = $this->request( $url, $parameters, "GET", $stringHttpHeader); break; 
			case 'POST' : $response = $this->request( $url, $parameters, "POST", $stringHttpHeader); break;
		}
	...
	*/
	...
	private function request( $url, $params=false, $type="GET", $httpHeader)
	{
		...
		if( $type == "GET" ){
			$url = $url . ( strpos( $url, '?' ) ? '&' : '?' ) . $urlEncodedParams . header($httpHeader);
		}
